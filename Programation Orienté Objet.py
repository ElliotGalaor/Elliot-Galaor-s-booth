...
def choix_limites_u(space,left,right,top,bottom):
    ...
    if(left.type=="D"):     #il y a 2 types de conditions aux limites, les limites de Dirichlet et de Neumann
        space.u[:,0]=left.value     #les limites de Neumann sont des valeurs de la dérivée spa de la variable (dépendante de la frontière) à la frontière
...
largeur=8       #largeur du domaine choisi selon y
...
mu=1.8*10**(-5)    #viscosité du fluide
...
while(t<time):
    ...
    timestep=cavity.dt
    choix_limites_u(cavity,D5,D1,D0,D0)
    choix_limites_v(cavity,D0,D0,D0,D0)
    choix_limites_pression(cavity,D5,D1,D1,D1)
    obtenir_vitesses_etoile(cavity,water)
    résoudre_Poisson(cavity,water,D5,D1,D1,D1)
