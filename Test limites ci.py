# -*- coding: utf-8 -*-
"""
Created on Sat May 21 13:41:19 2022
...
def choix_limites_pression(space,left1,right,top,bottom):
    if(right.type=="D"):
...
def résoudre_Poisson(space,fluid,left1,right,top,bottom):
    ...
    while(error>tol):
        ...
        choix_limites_pression(space,left1,right,top,bottom)   #on applique les conditions limites de la pression
...
while(t<time):
    ...
    choix_limites_pression(cavity,pgaucheN,pressureatm,pressureatm,pressureatm)
    cavity.u[:,1]=1
    cavity.p[:,1]=5
    obtenir_vitesses_etoile(cavity,water)
    résoudre_Poisson(cavity,water,pgaucheN,pressureatm,pressureatm,pressureatm)
