# -*- coding: utf-8 -*-
"""
Created on Sun Jun  5 15:03:36 2022
...
def choix_limites_pression(space,left1,left2,right,top,bottom):
    if(right.type=="D"):
        space.p[:,-1]=right.value
    elif(right.type=="N"):
    ...
    if(left1.type=="D"):
        space.p[:,0]=left1.value
    elif(left1.type=="N"):
        space.p[:,0]=-left1.value*space.dx+space.p[:,1]
    if(left2.type=="D"):
        space.p[:,1]=left2.value
    elif(left2.type=="N"):
        space.p[:,1]=-left2.value*space.dx+space.p[:,2]
...
def meth_obs(fonc):
    for k in range(ligneobs) :
        for j in range(colonneobs):
            fonc[k+coordyobs1][coordxobs1+j]=0
            fonc[k+coordyobs2][coordxobs2+j]=0
...
def résoudre_Poisson(space,fluid,left1,left2,right,top,bottom):
    ...
    while(error>tol):
        ...
        meth_obs(p)
        choix_limites_pression(space,left1,left2,right,top,bottom)   #on applique les conditions limites de la pression
...
colonnes=64*3    #nombre de point selon x dans la grille 
...
posx2=6
posy2=0
coordxobs1=int((posx1*colonnes)//longueur)
coordyobs1=int((posy1*lignes)//largeur)
coordxobs2=int((posx2*colonnes)//longueur)
coordyobs2=int((posy2*lignes)//largeur)
colonneobs=int((largeurobs*colonnes)//longueur)
ligneobs=int((longueurobs*lignes)//largeur)
print(ligneobs,colonneobs)

#Propriétés du fluide
rho=1               #densité du fluide
mu=1.8*10**(-1)     #viscosité du fluide
water=Fluid(rho,mu) #créer un objet de classe Fluid

#Propriétés des limites
ugauche=Boundary("D",1)
vgauche=Boundary("D",0)
uvertical=Boundary("N",0)
venteq=Boundary("D",0)
pgaucheN=Boundary("N",1)
pgaucheD=Boundary("D",5)
pressureatm=Boundary("D",1)
...
while(t<time):
    ...
    timestep=cavity.dt
    choix_limites_u(cavity,ugauche,venteq,uvertical,uvertical)
    choix_limites_v(cavity,vgauche,venteq,venteq,venteq)
    choix_limites_pression(cavity,pgaucheN,pgaucheD,pressureatm,pressureatm,pressureatm)
    obtenir_vitesses_etoile(cavity,water)
    résoudre_Poisson(cavity,water,pgaucheN,pgaucheD,pressureatm,pressureatm,pressureatm)
    ...
    u_mes.append(cavity.u_c[33,130])
    v_mes.append(cavity.v_c[33,130])
    p_mes.append(cavity.p_c[33,130])
...
def animate(i):
    ...
    cont=ax.contourf(X,Y,p_p)
    stream=ax.streamplot(X[::index_cut_y,::index_cut_x],Y[::index_cut_y,::index_cut_x],u_p[::index_cut_y,::index_cut_x],v_p[::index_cut_y,::index_cut_x],color="k")
    ax.add_patch(Rectangle((posx1,posy1),largeurobs,longueurobs,alpha=10))
    ax.add_patch(Rectangle((posx2,posy2),largeurobs,longueurobs,alpha=10))
    return cont,stream
...
plt.close('all')
img1=[img1[i] for i in range(len(img1)-1)]
vitesse=[(img2[i]**2+img3[i]**2)**(1/2) for i in range(len(img1))]
pression=[img4[i] for i in range(len(img1))]
