# -*- coding: utf-8 -*-
"""
Created on Fri Mar  4 15:46:06 2022
...
def résoudre_Poisson(space,fluid,left,right,top,bottom):
    ...
    error=1     #on définit l'erreur et la tolérance pour la convergence
    tol=1e-3
...
posx=8
posy=0
ligneobs=int(((longueurobs+posx)*lignes)//longueur)
colonneobs=int(((largeurobs+posy)*colonnes)//largeur)
...
if len(iterations)==0:
    ...
    final_iter=np.amax(iterations)          #discerne l'itération finale des intervalles
...
def animate(i):
    ...
    cont=ax.contourf(X,Y,p_p)
    stream=ax.streamplot(X[::index_cut_y,::index_cut_x],Y[::index_cut_y,::index_cut_x],u_p[::index_cut_y,::index_cut_x],v_p[::index_cut_y,::index_cut_x],color="k")
    ax.add_patch(Rectangle((posx,posy),largeurobs,longueurobs,alpha=10))
    return cont,stream
...
#Boyer's tips : On appelle ca des lignes de courants dans notre cas pas des lignes de champs :)
