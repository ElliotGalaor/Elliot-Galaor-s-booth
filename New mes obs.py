# -*- coding: utf-8 -*-
"""
Created on Tue May 31 16:23:38 2022
...
def meth_obs(fonc):
    for k in range(ligneobs) :
        for j in range(colonneobs):
            fonc[k+coordyobs][coordxobs+j]=0
...
posx=8
posy=0
coordxobs=int((posx*colonnes)//longueur)
coordyobs=int((posy*lignes)//largeur)
...
#    u_mes.append(cavity.u_c[16,130])
#    v_mes.append(cavity.v_c[16,130])
#    p_mes.append(cavity.p_c[16,130])
...
def animate(i):
    ...
    cont=ax.contourf(X,Y,p_p)
    stream=ax.streamplot(X[::index_cut_y,::index_cut_x],Y[::index_cut_y,::index_cut_x],u_p[::index_cut_y,::index_cut_x],v_p[::index_cut_y,::index_cut_x],color="k")
    ax.add_patch(Rectangle((8,posy),largeurobs,longueurobs,alpha=10))
    return cont,stream
...
"""                 #Création des courbes avec les valeurs des simulations
plt.close('all')
